...
public class WithdrawFundsTest {
    ...
    @Test
    public void testAWithdrawalForANonExistingAccount(TestContext context) {
        Async async = context.async();
        final String testWithdrawal = Json.encodePrettily(new Withdrawal(new BigDecimal("14.00"), new AccountID("unk4wn")));
                ...
                .handler(response -> {
                    ...
                    response.bodyHandler(body -> {
                        try {
                            ...
                            Assert.assertTrue(account.getBalance().compareTo(new BigDecimal(5)) == 0);
    ...
    public void testInsufficientFundsWithdrawal(TestContext context) {
        ...
        vertx.createHttpClient().put(port, "localhost", "/withdraw")
                ...
                .handler(response -> {
                    ...
                    context.assertTrue(response.headers().get("content-type").contains("application/json"));
                    response.bodyHandler(body -> {
                        Assert.assertTrue(body.toString().contains("This account does not have enough funds for this withdrawal"));
                        async.complete();
